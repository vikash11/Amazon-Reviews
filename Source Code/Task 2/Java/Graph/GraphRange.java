...
public class GraphRange
{
	...
	{
		try {
			FileReader tmp = new FileReader("%path to RangeTop reducers output%");
			...
			while ((sCurrentLine = tmpbr.readLine()) != null) {
				...
				for (int j = 0; j < allprod.size(); j++) {
					...
					File file = new File("graphs/range/"+category);
					if (!file.exists()) {
						if (file.mkdir()) {
					...
					File XYChart = new File( "graphs/range/"+category+"/"+asin+"rating.jpeg" ); 
